...
object RuleInspectionUtils {
    ...
    init {
        val config = ServiceManager.getService(TiadConfig::class.java)
        I18nResources.changeLanguage(config.locale)
    ...
    private fun loadAllTiadRule(): List<Rule> {
        try {
            ...
            logger.warn("no available rules")
            return emptyList()
